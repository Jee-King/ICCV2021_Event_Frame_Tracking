...
class ATOMProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            # data['train_event_stack'], _ = self.transform['joint'](image=data['train_event_stack'], bbox=data['train_anno'])
            ...
            # data['test_event_stack'], _ = self.transform['joint'](image=data['test_event_stack'], bbox=data['test_anno'], new_roll=False)
        ...
        for s in ['train', 'test']:
            ...
            crops_event = []
            # print('========>>>>>>>', type(data[s + '_images']), type(data[s + '_event_stack'][0][0]))
            for i in range(len(data[s + '_event_stack'][0])):
                crops_event_tem, _, _ = prutils.jittered_center_crop(data[s + '_event_stack'][0][i], jittered_anno, data[s + '_anno'],
                                                                    self.search_area_factor, self.output_sz)
                crops_event.append(crops_event_tem)

            # img3 = Image.fromarray(crops[0])
            # img4 = Image.fromarray(crops_event[0])
            # img3.save('/home/zjq/img3'+s + data['test_class'] +'.jpg')
            # img4.save('/home/zjq/img4'+s + data['test_class'] +'.jpg')
            # img1 = Image.fromarray(data['train_images'][0])
            # img2 = Image.fromarray(data['train_event_stack'][0])
            # img1.save('/home/zjq/img1'+ data['test_class'] + '.jpg')
            # img2.save('/home/zjq/img2'+ data['test_class'] + '.jpg')
            ...
            for i in range(len(crops_event)):
                ...
                data[s + '_event_stack'][0][i] = temp1[0]
                # data[s + '_event_stack'], _ = self.transform[s](image=crops_event, bbox=boxes, joint=False)
