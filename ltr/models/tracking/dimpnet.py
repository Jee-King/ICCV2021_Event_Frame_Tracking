...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.motion_attention_train = backbones.Motion_Attention()
        self.counter_guide_train = backbones.Counter_Guide()
        self.motion_attention_test = backbones.Motion_Attention()
        self.counter_guide_test = backbones.Counter_Guide()
    ...
    def forward(self, train_imgs, test_imgs, train_event_stack, test_event_stack, train_bb,  test_proposals, *args, **kwargs):
        ...
        train_event_feat1, train_event_feat2 = self.motion_attention_train(train_event_stack[0].reshape(-1, *train_event_stack[0].shape[-3:]).cuda(),\
                                                                     train_event_stack[1].reshape(-1, *train_event_stack[1].shape[-3:]).cuda(),\
                                                                     train_event_stack[2].reshape(-1, *train_event_stack[2].shape[-3:]).cuda())
        test_event_feat1, test_event_feat2 = self.motion_attention_test(test_event_stack[0].reshape(-1, *test_event_stack[0].shape[-3:]).cuda(),\
                                                                   test_event_stack[1].reshape(-1, *test_event_stack[1].shape[-3:]).cuda(),\
                                                                   test_event_stack[2].reshape(-1, *test_event_stack[2].shape[-3:]).cuda())

        [x1, x2] = self.counter_guide_train(train_feat['layer2'], train_feat['layer3'], train_event_feat1, train_event_feat2)
        train_feat['layer2'] = x1
        train_feat['layer3'] = x2

        [x3, x4] = self.counter_guide_test(test_feat['layer2'], test_feat['layer3'], test_event_feat1, test_event_feat2)
        test_feat['layer2'] = x3
        test_feat['layer3'] = x4
