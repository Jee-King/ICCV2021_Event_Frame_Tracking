...
class DiMP(BaseTracker):
    ...
    def extract_backbone_event_features(self, backbone_feat, event_im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            output_event_features0, output_event_features1 = self.net.motion_attention_test(event_patches[0], \
                                                                                       event_patches[1],\
                                                                                       event_patches[2])
            tmp1, tmp2 = self.net.counter_guide_test(backbone_feat['layer2'],
                                                backbone_feat['layer3'], \
                                                output_event_features0, output_event_features1)
            backbone_feat['layer2'] = tmp1
            backbone_feat['layer3'] = tmp2
    ...
    def generate_init_samples(self, im: torch.Tensor, event_im) -> TensorList:
        ...
        with torch.no_grad():
            ...
            output_event_features0, output_event_features1 = self.net.motion_attention_test(event_im_patches[0], \
                                                                                       event_im_patches[1],\
                                                                                       event_im_patches[2])
            tmp1, tmp2 = self.net.counter_guide_test(init_backbone_feat['layer2'],
                                                init_backbone_feat['layer3'], \
                                                output_event_features0, output_event_features1)
            init_backbone_feat['layer2'] = tmp1
            init_backbone_feat['layer3'] = tmp2
