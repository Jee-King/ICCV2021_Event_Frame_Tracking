...
class ATOMResNet18(MultiFeatureBase):
    ...
    def extract(self, im: torch.Tensor, event_im):
        im = im / 255
        im -= self.mean
        im /= self.std

        for i in range(len(event_im)):
            event_im[i] = event_im[i]/255
            event_im[i] -= self.mean
            event_im[i] /= self.std

        if self.use_gpu:
            im = im.cuda()
            event_im = [i.cuda() for i in event_im]

        with torch.no_grad():
            output_features = self.net.extract_features(im, self.feature_layers)
            output_event_features0, output_event_features1 = self.net.motion_attention(event_im[0], event_im[1], event_im[2])
            tmp1, tmp2 =  self.net.counter_guide(output_features[self.feature_layers[0]], output_features[self.feature_layers[1]],\
                                                     output_event_features0, output_event_features1)
            output_features[self.feature_layers[0]] = tmp1
            output_features[self.feature_layers[1]] = tmp2
