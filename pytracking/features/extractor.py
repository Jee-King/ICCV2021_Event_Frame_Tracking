...
class MultiResolutionExtractor(ExtractorBase):
    ...
    def extract(self, im, event_im, pos, scales, image_sz, return_patches=False):
        """Extract features.
        args:
            im: Image.
        ...
        patch_event_iter = []
        for single_event in event_im:
            temp_event_iter, _ = zip(*(sample_patch(single_event, pos, s*image_sz, image_sz, mode=self.patch_mode, \
                                                    max_scale_change=self.max_scale_change) for s in scales))
            patch_event_iter.append(temp_event_iter)

        im_patches = torch.cat(list(patch_iter))
        event_im_patches = [torch.cat(list(i)) for i in patch_event_iter]
        patch_coords = torch.cat(list(coord_iter))

        # im_patches = torch.cat([sample_patch(im, pos, s*image_sz, image_sz) for s in scales])

        # Compute features
        feature_map = TensorList([f.get_feature(im_patches, event_im_patches) for f in self.features]).unroll()
    ...
    def extract_transformed(self, im, event_im, pos, scale, image_sz, transforms):
        ...
        for event in event_im:
            temp_patch, _ = sample_patch(event, pos, scale*image_sz, image_sz)
            event_patch.append(temp_patch)
        # Apply transforms
        im_patches = torch.cat([T(im_patch) for T in transforms])
        event_patches = []
        for i in event_patch:
            temp_patches = torch.cat([T(i) for T in transforms])
            event_patches.append(temp_patches)
        # Compute features
        feature_map = TensorList([f.get_feature(im_patches, event_patches) for f in self.features]).unroll()
