...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # a1 = np.full((260, 346, 3),127, dtype='uint8')
        # event_image = [np.full((260, 346, 3),127, dtype='uint8'), np.full((260, 346, 3),127, dtype='uint8'), \
        #              np.full((260, 346, 3),127, dtype='uint8')]
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            event_img = [self._read_image(temp_path + '_1.jpg'), self._read_image(temp_path + '_2.jpg'), \
                           self._read_image(temp_path + '_3.jpg')]
            # event_img = self._read_image(frame_path.replace('/img/','/stack_img/'))
            # event_img = [np.full((260, 346, 3),127, dtype='uint8'), np.full((260, 346, 3),127, dtype='uint8'), \
            #                np.full((260, 346, 3),127, dtype='uint8')]
            start_time = time.time()
